...
/// 语音日记 ViewModel：
/// 按住说话（统一语音输入状态机）→ 松开后 STT 转写（按 SettingsStore.sttProvider 选服务）
...
final class VoiceDiaryViewModel {
    ...
    /// 语音输入统一状态机（录音/STT/会话生命周期，规则与聊天页一致）
    ...
    func stopRecordingAndProcess() {
        ...
        Task {
            ...
            guard let trimmed = await voiceInput.transcribe(capture.samples) else {
