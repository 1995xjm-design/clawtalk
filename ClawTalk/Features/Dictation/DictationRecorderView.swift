...
/// 口述录音页状态。
enum DictationRecorderState: Equatable {
    case idle
    case recording
    case transcribing
    case organizing
...
/// → 展示原始转写（可编辑）+「整理文档」（AI 优先，失败本地规则降级并诚实标注）
/// → 存 DictationStore。
@Observable
@MainActor
final class DictationRecorderViewModel {
    ...
    private(set) var state: DictationRecorderState = .idle
    var audioLevel: Float = 0
    var errorMessage: String? {
        didSet {
            if let errorMessage {
                LogCollector.record(module: "文档口述", errorMessage)
            }
        }
    }
    /// 原始转写（用户可编辑后整理）
    ...
    init(settingsStore: SettingsStore, dictationStore: DictationStore) {
        self.settingsStore = settingsStore
        self.dictationStore = dictationStore
    }

    // MARK: - 录音（与会议纪要/语音日记同模式）
        ...
        Task {
            ...
            do {
                ...
                if documentTitle.isEmpty {
                    documentTitle = Self.defaultTitle(from: trimmed, date: recordingStart ?? Date())
                }
                state = .idle
            } catch {
    ...
    func organizeDocument() {
        ...
        Task {
            let result = await DictationOrganizer.organize(
                transcript: trimmed,
                title: title.isEmpty ? nil : title,
                date: date,
                settings: settingsStore
            )
            dictationStore.add(result.note)
    ...
    private static func defaultTitle(from transcript: String, date: Date) -> String {
        ...
        return "\(formatter.string(from: date)) 口述：" + title
    }
...
/// - 有转写后显示「整理文档」按钮；AI 失败自动本地规则降级并诚实标注
/// - 整理完成弹出文档详情
struct DictationRecorderView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $viewModel.savedNote) { note in
            NavigationStack {
                DictationDetailView(note: note, store: viewModel.dictationStore)
            }
        }
    }
    ...
    private var contentArea: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                TextField("文档标题（可编辑，空则自动生成）", text: $viewModel.documentTitle)
                    .font(.headline)
                    .padding(12)
                    .background(
    ...
    private var transcriptSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if viewModel.transcript.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                Text("按住底部麦克风录音，松开后自动转写。转写后可以手动修改，再点「整理文档」。")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
            ...
            if !viewModel.transcript.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                Button {
                    ...
                    HStack(spacing: 8) {
                        ...
                        Text(viewModel.isOrganizing ? "整理中…" : "整理文档")
                            .font(.headline)
                    }
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 12)
                    .background(viewModel.isOrganizing ? Color.gray : Color.teal)
